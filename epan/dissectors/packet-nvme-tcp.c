                         ...
                         int offset)
{
    ...
    case nvme_fabrics_type_connect:
        ...
        break;
    default:
        proto_tree_add_item(data_tree, hf_nvme_tcp_unknown_data, nvme_tvb, 0,
                len, ENC_NA);
        break;
    }
}
                         ...
                         guint32 incapsuled_data_size)
{
    ...
    if (opcode == nvme_fabrics_command) {
        ...
        cmd_string = val_to_str(fctype, nvme_fabrics_cmd_type_vals,
                "Unknown FcType");
        ...
        if (incapsuled_data_size > 0) {
            dissect_nvme_fabric_data(tvb, nvme_tcp_tree, incapsuled_data_size,
                    cmd_ctx->fctype, offset + NVME_FABRIC_CMD_SIZE);
        }
        return;
    }
    ...
    if (incapsuled_data_size > 0) {
        ...
        nvme_data = tvb_new_subset_remaining(tvb, offset + NVME_CMD_SIZE);
                        ...
                        int offset)
{
    ...
    proto_item_append_text(ti, " (For Cmd: %s)",
            val_to_str(cmd_ctx->fctype, nvme_fabrics_cmd_type_vals,
                    "Unknown Cmd"));
                     ...
                     void* data _U_)
{
    ...
    case nvme_tcp_cmd:
        dissect_nvme_tcp_command(tvb, pinfo, tree, nvme_tcp_tree, ti, q_ctx,
                nvme_tcp_pdu_offset, incapsuled_data_size);
        break;
    case nvme_tcp_rsp:
